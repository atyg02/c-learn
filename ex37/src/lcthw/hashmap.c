...
/**
 * simple Bob Jenkins's hash algorithm taken fromthe 
		...
		uint32_t * hash_out)
{
	...
	// store it for the return to the caller can use it
	*hash_out = hash;
...
void *Hashmap_get(Hashmap * map, void *key)
{
	...
	HashmapNode *node = DArray_get(bucket, i);
	check(node != NULL,
		"Failed to get node from bucket when it should exits.");
...
void *Hashmap_delete(Hashmap * map, void *key)
{
	...
	if(ending != node) {
		// alright loook like it's not the last one, swap it
